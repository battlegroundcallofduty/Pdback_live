...
| 피드백페이지 1 | 피드백페이지 2 |
|:---------:|:---------:|
| ![피드백1](assets/feedback1.jpg) | ![피드백3](assets/feedback3.jpg) |
...
이 경험을 통해 데이터를 설계할 때는 수집 가능한 데이터를 먼저 파악한 뒤, 그중 사용자에게 실제로 의미 있는 데이터가 무엇인지를 한 번 더 가려내야 한다는 것을 배웠습니다. 또한 설계는 처음 계획대로 고정되는 것이 아니라, 페이지를 구현하는 과정에서 얼마든지 재설계될 수 있다는 것도 깨달았습니다.
 
**3. 코드리뷰로 배운 것**
 
담당 파트의 백엔드 이슈 4건 중 3건(인가 누락, N+1 · 메모리 적재, 에러 처리)이 모두 코드리뷰에서 나왔습니다.    
로컬에서 소량 데이터로 테스트할 때는 인지하지 못했던 부분이라, "동작한다"와 "안전하다 · 확장된다"는 다른 기준이라는 것을 깨달았습니다.
